...
对于这些组件的设计，我遵循的是低设计与高扩展原则。当然，这也只是我的设想，遵循程度与我的代码能力正相关。  
特征包括了：

* 只做基础功能。不会有复合功能类似于的工具箱这类的开源组件在组件中出现。
* 与业务无关。这点也是最重要的，所有的组件都一共轻松兼容绝大部分项目。
* 低依赖。尽量不使用三方依赖，这点主要是减少组件体量与功能拓展的实现。
